...
ggplot(ppr2, aes(x=price))+geom_density()+scale_x_continuous(labels=scales::dollar_format(prefix="€" ))

with(ppr2, summary(price))

ggplot(ppr2, aes(x=price/100))+geom_density()+scale_x_continuous(labels=scales::dollar_format(prefix="€" ))
...
ggplot(ppr3, aes(x=log(price)))+
    geom_density()+
    scale_x_continuous(
        labels=scales::dollar_format(prefix="€"))+xlab("log price, euros")
...
daily_data  <- ppr4 %>%
    ...
    summarise(count=n(), mean_price=mean(price, na.rm=TRUE),
              median_price=median(price, na.rm=TRUE),
              )
ggplot(daily_data, aes(x=date_of_sale, y=mean_price, size=count))+geom_point()
