...
* This Talk
- I will provide an introduction tools to improve the quality of code, and our
  confidence in it (examples in R)
- These tools are:
  - Automated testing
  - Refactoring
...
conda env export --name PPR-r > ppr_r.yml
#+end_src
...
- However, because we took a 5% sample earlier, we need to change
  things
* Handling the sample
...
#+end_src
- You can do this with *usethis*, but it was borked for me when I
  tried, so I did it manually
* Integration Test
...
#+end_src
#+begin_src R :session :tangle ppr/R/testing.R :exports none
   ##' .. content for \description{} (no empty lines) ..
   ##'
   ##' .. content for \details{} ..
   ##' @title save_test_data
   ##' @param obj 
   ##' @return 
   ##' @author richie
   ##' @export
...
#+end_src
* Abstract Saving files
- First, write a test
 #+begin_src R :session :tangle ppr/tests/testthat/test_load_save.R
   context("test saving and loading")
   data(ppr)
   ppr_test_load_save  <- ppr
   save_test_data(ppr_test_load_save)
   expect_equal(ppr,
                load_test_data("ppr_test_load_save_test_data.rds"))
 #+end_src
- This actually tests both functions, and handles the invariant we
  care about (that the saved data should be easily comparable to the test data)
* Write a function to make the test pass
 #+begin_src R :results none 
   save_test_data <- function(obj) {
     obj_name  <- substitute(obj)
     bpath  <- ""
     save_file_name  <- paste0(bpath,
                               obj_name,
                               "_test_data",
                               ".rds")
     saveRDS(obj, save_file_name)
 ...
 #+end_src
* Move existing functions to file
- Move the functions we have to a file (move function refactoring)
...
#+begin_src R :session :tangle ppr/R/ppr.R :exports code
...
* Generate Bootstrap Results :noexport:
...
* Write test for new function
  #+begin_src R :tangle ppr/tests/testthat/test_first.R
    test_df_output  <- load_test_data("../../inst/mark_values_as_large_test_data_done.rds")
    test_df_input  <- load_test_data("../../inst/mark_values_as_large_test_data.rds")
...
- This is most useful when you want to refactor code into different
  functions (because they do different things, or for naming purposes)
...
* What not to functionalise :noexport:
...
* Split Data Refactoring :noexport:
...
* Tests for our Data Analysis Functions
  #+begin_src R :session :tangle ppr/tests/testthat/test_funcs.R
    require(ppr)
    require(testthat)
...
* Yet More Tests
  ...
  #+end_src
* Conclusions
- Automated testing is a very, very useful practice
- It's a foundation for refactoring, which improves code quality immensely
- This talk has demonstrated how to get started
- Once you have tests, test-driven development is an extremely good practice [fn:10]
- I did a simpler [[https://github.com/richiemorrisroe/geocoder][example in Python]]
- And here's a [[https://github.com/richiemorrisroe/p0ker][poker simulation in python, which I've developed using TDD]]
- And [[https://github.com/richiemorrisroe/refactoring_tdd_presentation][the repository for this talk is here]]
* Further Work
- I haven't covered that much [[https://www.informit.com/store/refactoring-improving-the-design-of-existing-code-9780134757599][refactoring]]
- I haven't covered that much [[https://www.amazon.co.uk/Test-Driven-Development-Addison-Wesley-Signature/dp/0321146530][TDD]]
- I haven't covered that much [[https://www.amazon.co.uk/Working-Effectively-Legacy-Michael-Feathers/dp/0131177052/][working with legacy code]]
...
[fn:10] at least I have found it so 
