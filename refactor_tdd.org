...
#+end_src
#+begin_src R :session :tangle ppr/R/testing.R
   ...
   save_test_data <- function(obj) {
     ...
     cur_working_dir  <- getwd()
     split_wd  <- unlist(stringr::str_split(cur_working_dir, "/"))
     print(split_wd)
     if(tail(split_wd, n=1)=="inst") {
       bpath  <- ""
     }
     if(tail(split_wd, n=1)=="testthat") {
       bpath  <- "../../inst" 
     }
     else {
       stop("not sure where to save files")
       }
...
* Generate Bootstrap Results :nexport:
